...
const getApplicationTrackerDetails = async (isCount, status, limit, offset, keyword = "") => {
    try {
        if (isCount) {
            ...
            let query = `SELECT *, TO_CHAR(updated_at, 'DDth Mon YYYY') as updated_at FROM newbusiness.application_data WHERE status = $1 `;
            if (keyword) {
...
const getSequenceNextValue = async () => {
    try {
        const query = `SELECT nextval('newbusiness.create_lead_id')`;
...
const createUpdateApplication = async (appId = "", request) => {
    try {
        ...
        if (!appId) {
            const sequence = await getSequenceNextValue();
            let query = `INSERT INTO newbusiness.application_data (id, application_id, quote_id, lead_id, status, application_json, quote_json, requirement_json, premium) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`
            const res = await client.query(query, [sequence?.[0]?.nextval, applicationId, quoteId, leadId, status, applicationJson, quoteJson, JSON.stringify(requirementJson), premium]);
            return res?.rows || [];
        } else {
